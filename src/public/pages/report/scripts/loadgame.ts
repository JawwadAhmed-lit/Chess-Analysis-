...
async function fetchChessComGames(username: string) {
    ...
    try {
        ...
        $("#games-list").html("User Not found.");
    }
...
function fetchGames(username: string) {
    ...
    let selectedLoadType = $("#load-type-tab").val();
...
const loadTypeTab = $("#load-type-tab");
const usernameInput = $("#chess-site-username");

loadTypeTab.on("input", () => {
    const selectedLoadType = loadTypeTab.val();
